...
import wandb
from kd_experiments.datasets.imagenet import create_imagenet_dataset
from kd_experiments.kd_model_dino import FeatureKD
...
def main(args):
    ...
    train_dataset = create_imagenet_dataset(
        split="train", data_dir=args.data_dir, pretraining=True
    )
    ...
    model = FeatureKD(
        student_name=args.student, teacher_name=args.teacher, loss=args.loss
    )
    if args.teacher_checkpoint:
        model.teacher.load_state_dict(torch.load(args.teacher_checkpoint))

    optimizer = optim.AdamW(model.training_parameters(), lr=args.lr, weight_decay=5e-2)
...
if __name__ == "__main__":
    ...
    now = "dino_" + datetime.now().strftime("%Y-%m-%d-%H%M%S")
